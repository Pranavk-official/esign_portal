...
export function ApiKeysTable({
    ...
    const columns = useMemo<ColumnDef<ApiKeyResponse>[]>(() => [
        ...
        },
        {
            ...
            header: "Prefix",
            cell: ({ row }) => (
                <code className="bg-gray-100 px-2 py-0.5 rounded text-xs font-mono">{row.original.key_prefix}...</code>
            ),
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex flex-col text-xs">
                    <span className="text-gray-900">
                        {row.original.max_txn_count ? `${row.original.remaining_txn_count} / ${row.original.max_txn_count}` : "Unlimited"}
                    </span>
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <span className="text-sm">
                    {row.original.last_used_at ? format(new Date(row.original.last_used_at), "dd/MM/yyyy HH:mm") : "Never"}
                </span>
            ),
        },
        {
            ...
            <div className="flex flex-wrap items-center gap-4 bg-gray-50/50 p-4 rounded-lg border border-gray-100">
                <div className="flex items-center gap-4">
                    <Select
                        ...
                        onValueChange={(value) => onParamsChange({ ...params, environment: value === "all" ? undefined : value, page: 1 })}
                    >
